...
func newAgentEventService(app *App) *agentEventService {
...
func (s *agentEventService) SendTaskMessage(ctx context.Context, task db.Task, project db.Project, message string) error {
	...
	s.mu.Unlock()
	if err := s.app.store.UpdateTaskStatus(task.ID, db.StatusActive); err == nil {
...
func (s *agentEventService) clearTaskContext(ctx context.Context, task db.Task, project db.Project) error {
	...
	if err := s.app.store.UpdateTaskAgentStream(task.ID, &threadID, nil, &streamKind); err != nil {
		return err
	}
...
func (s *agentEventService) recordContextCleared(task db.Task) {
	_ = s.app.store.AppendTaskTranscriptMessage(task.ID, "status", "Context cleared.", nil, nil)
}
	...
	if task.AgentThreadID != nil && strings.TrimSpace(*task.AgentThreadID) != "" {
		...
		if resumed, err := client.ThreadResume(ctx, threadID); err == nil {
			...
			if task.AgentStreamKind == nil || strings.TrimSpace(*task.AgentStreamKind) == "" {
				if err := s.app.store.UpdateTaskAgentStream(task.ID, &threadID, task.AgentEventCursor, &streamKind); err != nil {
					return "", err
				}
			}
			s.rememberThread(task.ID, threadID)
...
func (s *agentEventService) StopTask(ctx context.Context, task db.Task) error {
	...
	if isClaudeTask(task.Agent) {
		...
		return s.app.store.UpdateTaskAgentStream(task.ID, nil, task.AgentEventCursor, nil)
	}
...
func (s *agentEventService) finishClaudeTurn(task db.Task, project db.Project, completedTurnID string, startQueued bool) {
	...
	refreshed, err := s.app.store.GetTask(task.ID)
	if err != nil {
		cancel()
		s.mu.Lock()
		delete(s.activeTurns, task.ID)
...
func (s *agentEventService) execClaudeStreamOnce(ctx context.Context, task db.Task, project db.Project, turnID, message string) error {
	registry := s.app.registryForProject(project.Path)
	...
	if waitErr == nil && endTurnSeen && !terminalSeen {
		...
		if event.Cursor != "" {
			cursor := event.Cursor
			streamKind := claudeStreamKind
			_ = s.app.store.UpdateTaskAgentStream(task.ID, &cursor, &cursor, &streamKind)
		}
	}
	if waitErr != nil {
...
func (s *agentEventService) forwardCodexEvents(client codexRuntime) {
	...
	for notification := range client.Events() {
		...
		task, err := s.app.store.GetTask(taskID)
		if err != nil {
			continue
		}
		...
		if event.Cursor != "" {
			cursor := event.Cursor
			_ = s.app.store.UpdateTaskAgentEventCursor(taskID, &cursor)
		}
		s.publish(taskID, event)
		if turnCompleted && completedCurrentTurn {
			_ = s.app.store.UpdateTaskStatus(taskID, db.StatusWaiting)
			s.app.emitMetadataEvent(task.ProjectID)
			s.app.syncDiscordAsync()
		}
	}
